...
Welcome to the Card contributor guidelines. We are _so happy to have you_. No matter who you are or what your contributing background is, we want this to be a safe, fun place for you to help out.
...
There is also a strict _no meanness_ rule. At every step, please be kind and inclusive, even if people are asking not-so-informed questions.
...
Install bourbon gem (4.2.7)
```shell
gem install bourbon -v 4.2.7
```

Install bourbon lib in the local project
```shell
bourbon install --path src/scss/bourbon
```
